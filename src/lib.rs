...
impl Quadkey {
    ...
    /// Calculate the map size based on the precision
    pub fn map_size(precision: usize) -> f64 {
        256.0 * (1 << precision) as f64
    }

    /// Calculate the ground resolution based on the latitude and precision
    ...
    /// Convert pixel values to coordinates
    ...
    /// Convert tile values to pixel values
    ...
    /// Convert tile values to a quadkey
    ...
    /// Convert a quadkey to tile values
